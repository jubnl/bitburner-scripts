...
`darknet/state.txt` on home, JSON, written only by the controller (write `state.tmp.txt`, then `state.txt`, keep `state.bak.txt`; validate on read, fall back to backup). Schema:
...
{ version: 1, resetTime: number, mode: "loot"|"labyrinth"|"balanced",
  ...
  stats:     { cracks: { [modelId]: { won, failed, attempts } }, ramFreed, cachesOpened, moneyFromCaches, phishMoney } }
```

`darknet/passwords.txt` (host to password, subset of state) and `darknet/cmd-<host>.txt` (per server: mode, claimed crack targets, worker allocation in threads per worker type, migration target, promote symbols, stasis flag, share flag, stop flag) are scp'd by the controller to every online known server each loop after `connectToSession`.
...
`solvers.js` exports `solve(details, attempt, options)` dispatching on `details.modelId`. `attempt(password)` resolves to `{success, feedback}` where `feedback` is the parsed heartbleed JSON (or the inline lab response). Budgets are hard caps; a solver returns `null` when exhausted or when feedback contradicts the model.
...
| NIL (Yesn_t) | probe `c.repeat(L)` per charset symbol, read per-position yes/yesn't | 11 numeric, 63 alnum |
| DeepGreen (Mastermind) | candidate set filtered by exact/misplaced counts, minimax pick | 30 |
| 2G_cellular (TimingAttack) | prefix oracle from "mismatch at (i)", build left to right | 10L numeric, 62L alnum |
| 110100100 | 8-bit binary groups to chars | 1 |
| OrdoXenos | XOR cipher with per-char 5-bit masks | 1 |
| BellaCuore (Roman) | decode; above difficulty 8 binary search on ALTUS NIMIS / PARUM BREVIS | 1 or 14 |
| AccountsManager_4.2 | binary search over `[0, 10^L)` on Higher/Lower | 4L |
...
| RateMyPix.Auth (Spice) | count oracle, vary one position at a time | 10L |
| PHP 5.4 (Sorted) | permutations of the sorted multiset; RMS deviation guidance when L >= 5 | 40 |
| OpenWebAccessPoint | regex `host:password` up to difficulty 16; intersect captures above | 1 or 8 |
...
Scarce resources: stasis assignment respects the limit and removes links no longer useful; migration targets get charging agents on all their neighbours; `--allow-webstorm` (default false) permits `unleashStormSeed` in loot mode when the frontier has been stale for an hour; never manual backdoors.

Labyrinth: when a lab is adjacent to a pinned server with enough free RAM, launch `--lab-walkers` (default 3) instances of `lab.js` with `--lab-threads` (default 6). On completion the walker execs the agent on the lab, the agent opens the cache, the controller records `rewardQueuedAt` and pauses lab spending until the reset time changes.
