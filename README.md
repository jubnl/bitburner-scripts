...
- `darknet.js` - **Darknet (3.0)** - Fully automates the Darknet: cracks server passwords, spreads thin agents, frees blocked RAM, opens caches, phishes, promotes stocks, migrates servers across the air gaps and walks the labyrinths for their reward augmentations (including The Red Pill). Modes are set with `--mode loot|labyrinth|balanced` (loot chases money and caches, labyrinth pushes toward the current lab, balanced switches between them as charisma and network depth allow). `--status` prints a summary of what it knows and exits, `--kill` stops the whole fleet, `--allow-webstorm` permits `unleashStormSeed` when the frontier goes stale, and `--port` picks the netscript port the agents report on (default 15). `daemon.js` launches it automatically once you own `DarkscapeNavigator.exe` (or are in BN15 / own SF15), unless you pass `--no-darknet`. A `--mode` given on the command line pins the mode for the whole run; the config file `darknet.js.config.txt` can switch the mode at runtime only when `--mode` was **not** passed. Note that charisma is the pacing resource: most password models and every labyrinth are gated behind a charisma requirement, so early progress is slow until your charisma catches up (`work-for-factions.js` and `sleeve.js` will train toward the goal it publishes).
