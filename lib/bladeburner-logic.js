...
/** Run Diplomacy when it pays for itself: the time it takes to bring chaos down to chaosThreshold (e.g. --chaos-recovery-threshold) is less than
 * the rank-time the game's fixed chaos-difficulty cliff (Action.ts getChaosSuccessFactor, always sqrt(1 + chaos - 50) - a constant 50, not any
 * script option) would waste over the expected stay. With chance divided by mult, the level model (bladeburner.js getTargetLevel) loses at least
 * the fraction 1 - 1/mult of the rank rate (the real loss is larger: levels drop by log(mult)/log(difficultyFac) and reward with them), so this is
 * a conservative test. chaosThreshold only selects the Diplomacy target (how far to bring chaos down), never the difficulty cliff itself. */
export function shouldRunDiplomacy(chaos, chaosThreshold, charisma, expectedStayMinutes) {
    if (chaos <= chaosThreshold) return false;
    const lostFraction = 1 - 1 / chaosDifficultyMult(chaos); // always the game's fixed 50 cliff, independent of chaosThreshold
...
export function planSkillUpgradeCount(costForOne, costForTwo, adjustment, nextBestPerceivedCost, unspent, maxCount = Infinity) {
    if (!(costForOne > 0) || costForOne > unspent || maxCount <= 0) return 0;
