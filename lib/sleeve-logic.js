...
/** Note (SL-1): the ratio this gate uses is Powerhouse Gym's, but sleeve.js also applies it to the university classes
 * (algorithms / leadership, including the --darknet-charisma path). University cost/exp ratios are roughly 3x cheaper
 * per point of exp, so for those call sites the gate is about 3x stricter than the cost ceiling the user asked for.
 * The error is one-directional - it only ever declines training that would in fact have been affordable, never the
 * reverse - so it is left as is. Give this a costPerExp (or cost/exp multiplier) argument if that ever needs fixing.
 * @param {number} shock
